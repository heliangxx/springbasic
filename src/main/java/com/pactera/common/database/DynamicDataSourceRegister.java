...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    private Logger logger = LogManager.getLogger(DynamicDataSourceRegister.class);

    //指定默认数据源(springboot2.0默认数据源,如想使用其他数据源可以自己配置)
    ...
    private void initslaveDataSources(Environment env) {
        ...
        if(dsPrefixs!=null && dsPrefixs.trim().length()>0){
